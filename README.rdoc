= Quorum {<img src="https://secure.travis-ci.org/ncgr/quorum.png?branch=gmap" alt="Build Status" />}[http://travis-ci.org/ncgr/quorum]
...
Build your GMAP database(s) using the rake task below.

  rake quorum:gmapdb:build

Arguments:

* DIR= - path to {.fa, .fa.gz} files containing raw data. Separate multiple \
  directories with a colon (:).

  DIR=/path/to/dir:/path/to/another/dir

* REBUILD_DB= - removes existing GMAP database(s) before building {true or
  false}. Defaults to false.

* EMPTY= - skip gmap_build and create necessary directories {true or false}.
  Defaults to false. Set this argument to true if you wish to create your own
  GMAP database(s).

Example:

  rake quorum:gmapdb:build DIR=/path/to/dir:/path/to/another/dir \
  REBUILD_DB=true
